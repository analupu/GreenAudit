<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        $users_query = mysqli_query($con, "SELECT `id`, `email`, `password` FROM `users` WHERE `email` = '" . $form_email . "' AND `password` = '" . $form_password . "'");

        $error = false;
        if (mysqli_num_rows($users_query)) {
            $_SESSION['audit_logged_in'] = true;
            $_SESSION['audit_logged_in_user_id'] = mysqli_fetch_assoc($users_query)['id'];
